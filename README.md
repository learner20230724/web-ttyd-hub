...
在终端内向上滚动，自动打开**历史输出**：可以继续滚轮上下滚动、拖动滚动条、选中文字复制；手机可点「历史输出」后滑动。点击「回到终端」或按 Esc 继续输入，终端保持连接。历史视图读取 tmux 当前窗格保留的内容（最多请求前 20,000 行），不发送按键、不切换 tmux 复制模式，不影响其他浏览器。

在历史内容区域直接打字或粘贴，会自动回到终端并转交这次输入；中文输入法在选词完成后转交，复制和滚动仍留在历史中。粘贴沿用终端的括号粘贴处理，不额外发送回车。

历史是打开时的快照，点击「刷新」获取新内容；回到终端后向下滚动不会再发送方向键。Ctrl+滚轮保留浏览器缩放。历史接口沿用整站认证，响应禁止缓存。已被 tmux 丢弃、被应用清屏覆盖或未进入历史缓冲的内容无法找回。
...
顺序和置顶状态保存在当前浏览器的 localStorage，刷新后保留，同源的其他标签页自动同步；不同浏览器或设备不共享排列。排序和置顶不会切换当前会话或重建终端连接。
...
- 浏览器回滚与 tmux 历史是两套机制。机器重启、进程退出或使用 systemd 默认进程组清理都可能终止任务。
...
会话卡片仅显示一个「⋯」操作菜单，名称以 12px 完整换行。手机顶部「✥」控制常驻的六键面板（↑ ↓ ← → ↵ Shift+←），连续按键不会收起，回车不发送草稿。以上功能在线更新，无需新的 APK。
