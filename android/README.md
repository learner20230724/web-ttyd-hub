...
1. 输入你的服务器地址（完整 `https://域名` 或 `http://IP:端口`）。成功连接后加入服务器列表，下次启动自动回到最近成功连接的服务器。
2. 如果服务设置了 HTTP Basic 认证，弹窗输入与网页相同的用户名和密码。安装包没有内置服务器地址或密码，用户名与密码由 Android Keystore 的 AES-GCM 密钥加密后保存在本机；下次自动登录，密码失效时重新输入。返回键菜单可清除当前服务器保存的登录，其他服务器不受影响。
3. 选择或创建终端，使用原网页的全部会话功能。新建时终端类型默认 Codex，自动运行 `codex --yolo`，也可改选 Bash 等普通终端。下次打开自动回到上次选中的会话；原会话已删除或归档时展开列表供选择。
4. 要连接另一台机器，按 Android 返回键打开「连接选项 → 添加服务器」。输入地址并登录一次，以后直接点「已保存的服务器」里的地址切换，不必退出或反复输入密码。

手机操作：

- 手机使用独立阅读视图，不建立会改变共享终端尺寸的 ttyd 连接；同时使用电脑时，电脑窗口不会被手机挤窄。
- Codex 对话按 Markdown 排版，代码块可横向滚动；普通终端保留 ANSI 颜色。上下滑动阅读，长按文字选择/复制。
- 顶部标题旁的绿色数字表示其他会话中已完成但未读的数量，排除当前会话与归档；点击数字按列表顺序直接打开最上方的未读会话，可连续点击依次查看，全部读完后隐藏。
- 底部输入框点击即弹出系统键盘，长按粘贴，支持中文与多行文字；「发送」提交，「＋ → 仅输入」只粘贴不回车。「✥」面板提供上下左右、回车、Shift+← 和 ESC，点击不会发送草稿或自动收起；「＋」保留 Esc、Tab 和中断。
- 已去掉 APK 原生顶部工具栏。Android 返回键打开连接选项，列表中标出当前服务器，可直接切换、添加地址、重新加载或返回桌面。
...
覆盖安装后首次输入账号密码，后续重新打开自动登录。凭据按服务器协议、域名、端口和认证域隔离，仅保存于本机，不上传、不进入网页存储；外部资源不能接收登录凭据。1.5 起返回键菜单的「清除当前服务器登录」只删除该服务器保存的账号密码。原生更新仍沿用包名和签名，保留阅读设置。
...
「连接选项」与添加地址页面都显示已保存的服务器，按最近成功使用排序，当前连接标为「当前」。点另一台直接连接；点当前项只关闭菜单，避免无意义的重新加载。失败的新连接不会替换上次成功的启动地址。每台机器继续运行各自的 Hub、tmux 和 Codex，切换客户端不会终止任何服务器任务，不合并不同机器的会话列表。

升级自动保留旧版的服务器地址与加密密码。后续成功连接的地址自动加入列表；服务器协议、主机、端口和入口路径用于去重，密码仍沿用原来的协议、主机、端口和认证域隔离。各站点的登录、已读标记、会话选择与内容缓存保留在各自 origin；密码没有进入地址列表或网页存储。

每行「⋯ → 从列表移除」只移除本机快捷入口，不退出登录、删除内容或杀死任务。需要忘记密码时使用「清除当前服务器登录」，其他服务器的加密凭据保留。WebView 的 HTTP 认证可能在进程内暂时缓存；清除保存密码不是服务端注销，参见 [Android 的说明](https://developer.android.com/reference/android/webkit/WebViewDatabase#clearHttpAuthUsernamePassword())。
